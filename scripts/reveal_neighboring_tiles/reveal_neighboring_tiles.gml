function reveal_neighboring_tiles(row, column) {
	for (var row_neighbor = -1; row_neighbor <= 1; row_neighbor++) {
		...
		for (var column_neighbor = -1; column_neighbor <= 1; column_neighbor++) {
			...
			if column_scan < 0 or column_scan >= obj_minefield.NUMBER_OF_COLUMNS or obj_minefield.minefield_display[row_scan][column_scan] != "0" {
				continue;
			} else if obj_minefield.minefield_blueprint[row_scan][column_scan] == "0" {
